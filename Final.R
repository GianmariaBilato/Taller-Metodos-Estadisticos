...
plotmeans(utilidadPer ~ PIB, main="Heterogeineity across PIB", data=Datap_final)
plotmeans(utilidadPer ~ Inflacion, main="Heterogeineity across Inflacion", data=Datap_final)
...
coef(mixed)
yhat2<-fitted(mixed)
scatter3d(yhat2, PIB*coef(mixed) ,Infalcion, data=Datap_final,groups=factor(Datap_final$NIT),surface.col = 1:18, boxplots=FALSE, smooth=TRUE, reg.line=FALSE)
